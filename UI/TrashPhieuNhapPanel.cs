    ...
    public class TrashPhieuNhapPanel : UserControl
    {
        ...
        public TrashPhieuNhapPanel()
        {
            ...
            btnRestore.Click += (s, e) => {
                ...
                string id = dgv.CurrentRow.Cells["IdPN"].Value.ToString();
                if(controller.Restore(id)) { MessageBox.Show("✅ Khôi phục thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); LoadData(); }
            };
            ...
            btnDel.Click += (s, e) => {
                if (dgv.CurrentRow == null) return;
                string id = dgv.CurrentRow.Cells["IdPN"].Value.ToString();
                if(MessageBox.Show("⚠ Xóa vĩnh viễn phiếu nhập?", "Cảnh báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
